...
The plugin chain is composed of four layers, applied in this order:

- Default app plugins: These are the plugins that Sloth will execute by default on all SLOs (Sloth SLO validation and rule generation core logic).
- App plugins: These are the plugins that Sloth will execute on all SLOs, these plugins target the same SLOs as the default plugins.
...
The SLO plugin chain will be generated by aggregating plugins in order: `Default app plugins` +  `App plugins` + `SLO group plugins` + `SLO plugins`:
...
!!! Info "Override all default app plugins logic"

    In case you want to rewrite all the default app plugins logic from Sloth, you could disable the default app plugins and configure your own App plugins. Use these two flags: `--slo-plugins` and `--disable-default-slo-plugins`.

    Check this [F.A.Q question](../faq/index.md#can-i-override-the-full-sloth-slo-plugin-logic).
...
Sloth comes with some built-in plugins already loaded. Check them [in the docs catalog](../slo-plugins/core/debug_v1.md)
